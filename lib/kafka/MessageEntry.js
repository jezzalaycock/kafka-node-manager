                ...
                if (self.firstTime){
                    ...
                    self.messageManager.offsetService.fetch([request], function (err, offsets) {
                        if (err)
                            ...
                            var fetchedOffset = offsets[self.topic][self.partition][offsets[self.topic][self.partition].length - 1] - 1;
            ...
            // Offsets are invalid - need to retrieve the earliest value from the partition
                ...
                self.messageManager.offsetService.fetch([request], function (err, offsets) {
                    if (err)
                        ...
                        self.currentOffset = offsets[self.topic][self.partition][offsets[self.topic][self.partition].length - 1] - 1;
        ...
        if (err) (cb(err));
            ...
            if (!self.stale && self.toCommitOffsets.length
                ...
                if (self.toCommitOffsets.length > 0) {
                    var newHighWaterMark = self.toCommitOffsets[self.toCommitOffsets.length - 1].offset - 1;
                    var commitList = []
                    var poppedOffset = self.toCommitOffsets.pop();
                    while (poppedOffset != undefined && (poppedOffset.offset == newHighWaterMark + 1)) {
